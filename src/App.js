...
export default function App() {
  ...
  const getCardPower = (card, currentTrump, leadSuit) => {
    ...
    // Se for trunfo, ganha bônus máximo
    if (card.suit === currentTrump) {
      power += 200;
      // Manilhas de trunfo continuam sendo as mais fortes
      if (card.label === "7") power += 50;
      if (card.label === "3") power += 49;
      if (card.label === "2") power += 48;
    }
    // Se seguir o naipe da puxada, ganha bônus médio
  ...
  const getCardPoints = (card, currentTrump) => {
    ...
    // Figuras valem pontos SEMPRE (qualquer naipe)
    if (label === "K") return 4;
    if (label === "J") return 3;
    if (label === "Q") return 2;

    // Ás e 7 também valem pontos em qualquer naipe
    if (label === "A") return 11;
    if (label === "7") return 10;

    // Cartas que SÓ valem pontos se forem TRUNFO
    if (isTrump) {
    ...
    if (newScores[winnerId] >= POINTS_GOAL) {
      ...
      setTimeout(() => {
        ...
        if (nextDeck.length > 0) {
          ...
          if (pWinner) {
            const hWinner = pWinner.getState("hand") || [];
            pWinner.setState("hand", [...hWinner, c1]);
          }

          if (nextDeck.length > 0) {
            const c2 = nextDeck.shift();
            const loserId = currentP.find((p) => p.id !== winnerId)?.id;
            const pLoser = currentP.find((p) => p.id === loserId);
            if (pLoser) {
              const hLoser = pLoser.getState("hand") || [];
              pLoser.setState("hand", [...hLoser, c2]);
            }
          }
          setDeck(nextDeck);
        }
